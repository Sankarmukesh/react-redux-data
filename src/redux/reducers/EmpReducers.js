...
export const EmpReducers = (state = intialState, actions) => {
  switch (actions.type) {
    ...
    case "EDIT":
      ...
      axios.put(`http://localhost:3000/employees/${actions.payload.id}`,actions.payload.emp).then((res) => {
      
      });
      
      return {...state,emp_data:state.emp_data.map(e=>e.id===actions.payload.id ? actions.payload.emp : e)};
    case "Delete":
       
      axios.delete(`http://localhost:3000/employees/${actions.payload.id}`).then((res) => {
       
      });
      
      return {...state,emp_data:state.emp_data.filter(e=>e.id!==actions.payload.id)};
    default:
      return state;
  }
};
